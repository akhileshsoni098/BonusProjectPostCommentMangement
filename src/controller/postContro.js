...
const getPost = async function (req, res) {
  try {
    ...
    if(!ExistId){return res.status(404).send({status:false , message:"post id doesn't exist"})}
    
    if(!isValidObjectId(postId)){return res.status(400).send({status:false , message:"Invailed post id"})}

    const PostDetails = await PostModel.findOne({_id:postId,isDeleted:false}).select({
      name: 1,
      content: 1,
      title: 1,
      createdAt: 1,
    });
    if(!PostDetails){return res.status(400).send({status:false , message:"Invailed entry"})}
    const commentData = await CommentModel.find({ post: postId , isDeleted:false})
...
const updatePost = async function (req, res) {
    try {
      ...
      if (Object.keys(data).length == 0) {
        return res
          .status(400)
          .send({ status: false, message: "please provide some field to update" });
      }
      ...
      const upPost = await PostModel.findOneAndUpdate({_id:postId},{$set:{name:data.name, title:data.title , content:data.content}},{new:true})
...
const deletePost = async function(req,res){
  ...
  await PostModel.findByIdAndUpdate({_id:postId, isDeleted:false},{isDeleted:true})
